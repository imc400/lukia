...
Responde SOLO con el JSON.`;
      ...
      const trustScore = JSON.parse(content);
      
      // Validar valores
      Object.keys(trustScore).forEach(key => {
        if (key !== 'explanation' && typeof trustScore[key] === 'number') {
          const value = trustScore[key];
          trustScore[key] = Math.max(0, Math.min(10, value));
        }
      });
   ...
   */
  async translateAndSimplify(text, targetLanguage = 'es') {
